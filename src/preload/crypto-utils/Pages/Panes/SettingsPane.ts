...
export class SettingsPane extends Pane {
	constructor(container_: Container) {
		...
		// Add listeners
		const select = $("settings_select_theme") as HTMLSelectElement;
		...
		const password_settings_changed = $$(["settings_password_length", "settings_password_include_numbers", "settings_password_include_symbols", "settings_password_include_lowercase", "settings_password_include_uppercase"]);
	...
	updatePane(data?: any): void {
		...
		for (let index = 0; index < container.getSlots().length; index++) {
			...
			const askString = "Are you certain that you want to delete slot {}? The person using slot {} will not be able to log in anymore.";
			...
			// make icon
			const containerImage = document.createElement("i");
		...
		const containerSettings = container.settings == null ? new Settings() : container.settings;
	...
	addSlot() {
		...
		// get passwords
		const password_once = $("password_new_slot_once") as HTMLInputElement;
		const password_twice = $("password_new_slot_twice") as HTMLInputElement;
	...
	changePassword() {
		...
		// get passwords
		const password_once = $("password_change_once") as HTMLInputElement;
		const password_twice = $("password_change_twice") as HTMLInputElement;
	...
	updateTheme() {
		...
		setTimeout(() => {
			// theme change
			const settingsObject = container.settings == null ? new Settings() : container.settings;

...
class CreateContainer extends Pane {
	constructor(container_ : Container) {
		...
		// submit button
		const createContainer = this;
		$("create_container_submitButton").addEventListener("click", () => createContainer.submitListener());
		container = container_;

		// Add argon2 specific listeners
		const argonElements = $$(["create_container_kdf_argon2", "create_container_kdf_PBKDF2", "create_container_argon2_auto_memory"]);
   ...
   */
	argon2_options_listener() {
		const kdf_argon2 = $("create_container_kdf_argon2") as HTMLInputElement;
		const argon2_memory = $("create_container_argon2_memory") as HTMLInputElement;
		const argon2_auto_memory = $("create_container_argon2_auto_memory") as HTMLInputElement;
   ...
   */
	time_listener() {
		...
		const timeInSeconds = ($("create_container_time") as HTMLInputElement).value;
   ...
   */
	async submitListener(): Promise<void> {
		...
		// get all inputs
		// get ciphers
		const cipher_blowfish = $("create_container_cipher_blowfish") as HTMLInputElement;
		const cipher_aes = $("create_container_cipher_aes") as HTMLInputElement;

		// Get KDFs
		const kdf_argon2 = $("create_container_kdf_argon2") as HTMLInputElement;
		const kdf_PBKDF2 = $("create_container_kdf_PBKDF2") as HTMLInputElement;

		// Get memory and time
		const time = $("create_container_time") as HTMLInputElement;
		let memory = calculateMemory(); // memory in MB


		// get passwords
		const password_once = $("create_container_password_once") as HTMLInputElement;
		const password_twice = $("create_container_password_twice") as HTMLInputElement;
		...
		const password = password_once.value;
		...
		const keySize = algorithm != "Blow" ? 32 : 56;
		...
		const targetTime = Number.parseFloat(time.value) * 1000;
		const timeScale = targetTime / timeTaken;
		...
		// Make container data
		const ivSize = algorithmIvBytes(algorithm);
		const containerIv = getRandomBytes(ivSize);
		...
		const masterKey = Crypto.randomBytes(keySize);

		const defaultIdentity = new Identity(identityData);
		...
		// Test contaner data
		const ids = container.getIdentites();
		console.log(ids);
...
function disableEverything() {
	...
	disableStatus(objects as Array<HTMLInputElement>, true);

	const benchmarkScreen = $("create_container_benchmarkScreen");
...
function calculateMemory() {
	// Find system memory
	const memory_amount_notifier = $("create_container_memory_amount_notifier");
	const argon2_memory = $("create_container_argon2_memory") as HTMLInputElement;
	const memory_label_string = "We will use {} of memory.";
	...
	if (!($("create_container_kdf_argon2") as HTMLInputElement).checked) {
		...
		const memoryInGB = (navigator as any).deviceMemory as number; // manual override for TS

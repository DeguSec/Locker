...
function updateIdentityPane() {
	...
	// get identities
	const identities = container.getIdentites();
	const CurrentIdentity = identities[currentIdentity];
	...
	const identity_select = $("identity_select") as HTMLSelectElement;
...
function addIdentity() {
	const identityName = ($("add_identity_form_name") as HTMLInputElement).value;
	const identityDesc = ($("add_identity_form_desc") as HTMLInputElement).value;
	...
	const identityObject = new Identity(JSON.stringify(identityData));
	try {
